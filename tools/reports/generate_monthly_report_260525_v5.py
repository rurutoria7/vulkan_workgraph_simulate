...
VERIFY_DIR = MONTHLY / "verification" / "260525_optimization_evaluation_v5"
ASSET_DIR = VERIFY_DIR / "assets"

CSV_OUT = METRICS / "monthly_v5_normalized_measurements_20260525.csv"
NOTES_OUT = METRICS / "monthly_v5_measurement_notes_20260525.md"
PPTX_OUT = DECK_DIR / "260525_MTK_WorkGraph_VulkanPersistentThread_POC_OptimizationEvaluation_v5.pptx"
...
def write_measurement_files(rows: list[dict[str, str]]) -> None:
    ...
    notes = f"""# Monthly v5 Measurement Notes
...
def choose_font(size: float, bold: bool = False) -> str:
    return FONT_DISPLAY if bold and size >= 10 else FONT_BODY


def textbox(slide, x, y, w, h, text, size=14, color=C.BODY, bold=False, align="left", margin=0.05, font_face=None):
    ...
    p.font.name = font_face or choose_font(size, bold)
    p.font.size = Pt(size)
    p.font.bold = bold
...
def header(slide, title: str, page: int):
    ...
    divider = slide.shapes.add_connector(1, Inches(0.45), Inches(0.98), Inches(12.88), Inches(0.98))
    divider.line.color.rgb = rgb(C.LINE)
...
if (atomicCompSwap(control.q2[shard].count, cur, cur - claim) == cur) {
    ...
    prs.save(PPTX_OUT)
    scrub_unused_arial(PPTX_OUT)


def scrub_unused_arial(pptx_path: Path) -> None:
    """Keep PowerPoint theme defaults aligned with the explicit slide fonts."""
    ...
    with zipfile.ZipFile(pptx_path, "r") as zin, zipfile.ZipFile(tmp, "w", zipfile.ZIP_DEFLATED) as zout:
        for item in zin.infolist():
            ...
            if item.filename.endswith(".xml"):
                data = data.replace(b"Arial", FONT_BODY.encode("utf-8"))
