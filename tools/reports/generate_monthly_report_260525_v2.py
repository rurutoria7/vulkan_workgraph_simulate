...
VERIFY_DIR = MONTHLY / "verification" / "260525_optimization_evaluation_v2"
ASSET_DIR = VERIFY_DIR / "assets"

CSV_OUT = METRICS / "monthly_v2_normalized_measurements_20260525.csv"
NOTES_OUT = METRICS / "monthly_v2_measurement_notes_20260525.md"
PPTX_OUT = DECK_DIR / "260525_MTK_WorkGraph_VulkanPersistentThread_POC_OptimizationEvaluation_v2.pptx"
...
def write_measurement_files(rows: list[dict[str, str]]) -> None:
    ...
    notes = f"""# Monthly v2 Measurement Notes

Date: 2026-05-25

This file defines the measurement rules used by the v2 monthly deck.

## Rules

- Headline performance numbers use median clean `compute_ms` after warmup.
- Average is retained as secondary context and is not used for headline speedup.
...
- RGP occupancy is auxiliary context. `wavefront = 0` is not used as proof that the dispatch ended or that the only stall source was memory/atomic work.
...
The deck cites `{rel(CSV_OUT)}` and this notes file as its numeric source. The CSV contains the source file and warmup rule for each row.
"""
...
def create_assets(rows: list[dict[str, str]]) -> dict[str, Path]:
    ...
    create_chart(
        ...
        "Median clean compute_ms by optimization stage",
...
def header(slide, title: str, page: int):
    ...
    textbox(slide, 9.2, 7.12, 3.3, 0.18, "Optimization Evaluation v2", size=7, color=C.MUTED, align="right")
...
def build_deck(rows: list[dict[str, str]], assets: dict[str, Path]) -> None:
    ...
    textbox(slide, 0.75, 2.35, 10.8, 0.52, "May 2026 optimization evaluation: validating global atomic traffic as the queue bottleneck", size=18, color=C.BODY, bold=True)
    metric_box(slide, 0.75, 3.25, 2.45, 1.35, f"{baseline:.2f} ms", "Baseline median", "problem", "clean compute_ms")
    metric_box(slide, 3.55, 3.25, 2.45, 1.35, f"{final:.2f} ms", "Final median", "adopted", "main integrated")
    metric_box(slide, 6.35, 3.25, 2.45, 1.35, f"{speedup:.1f}x", "Median speedup", "evidence", "same workload")
    textbox(slide, 0.75, 5.1, 10.7, 0.62, "Adopted path: Q1/Q2 queue sharding + Q1 lane-pop. Request batching is tested, documented, and not adopted for the current protocol/workload.", size=15, color=C.BODY)
    textbox(slide, 0.75, 6.85, 3.2, 0.2, "AMD RX 7900 XTX / RDNA 3", size=8, color=C.MUTED)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Executive summary: hypothesis to validated bottleneck", page)
    cols = [
        ("Hypothesis", "The persistent-thread queue architecture is atomic-heavy, so global atomic traffic was suspected.", "problem"),
        ("Evidence", "Clean timing plus counter-enabled runs identify queue contention, CAS failures, backlog, and ready-spin behavior.", "evidence"),
        ("Adopted path", "Q1 sharding, Q2 sharding, and Q1 lane-pop reduce shared atomic hotspots.", "adopted"),
        ("Result", f"Median clean compute drops from {baseline:.2f} ms to about {final:.2f} ms on the final integrated path.", "adopted"),
    ]
    ...
    textbox(slide, 0.7, 5.35, 11.7, 0.58, "Performance results come from clean timestamp runs; atomic counters are used to explain the bottleneck behavior.", size=13.5, color=C.INK, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Recap architecture: persistent workers and atomic queues", page)
    y = 2.0
    ...
    textbox(slide, 0.9, 3.85, 4.9, 0.32, "Why atomic traffic is plausible", size=15.5, color=C.ORANGE, bold=True)
    multi_text(slide, 0.9, 4.28, 4.9, 0.8, ["Cross-workgroup handoff uses atomics for correctness on RDNA 3.", "Queue state and ready flags are shared by many lanes and workgroups."], size=12, bullet=True)
    rect(slide, 6.75, 3.65, 5.55, 1.55, fill=C.WHITE, line=C.LINE)
    textbox(slide, 7.0, 3.85, 4.9, 0.32, "What v2 validates", size=15.5, color=C.BLUE, bold=True)
    multi_text(slide, 7.0, 4.28, 4.9, 0.8, ["Metrics confirm queue contention instead of treating it as an assumption.", "Optimizations that split or diversify queue access dominate the gains."], size=12, bullet=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Prior hypothesis: suspected, not yet proven", page)
    rect(slide, 0.7, 1.45, 5.55, 4.25, fill=C.WHITE, line=C.LINE)
    semantic_tag(slide, 0.95, 1.72, "PRIOR REPORT", "neutral", w=1.25)
    textbox(slide, 0.95, 2.18, 4.9, 0.42, "Known state", size=18, color=C.INK, bold=True)
    multi_text(slide, 0.95, 2.75, 4.8, 1.55, ["Correctness-first POC was working.", "The architecture depended heavily on atomics.", "Performance investigation was not complete."], size=13.5, bullet=True)
    rect(slide, 7.0, 1.45, 5.55, 4.25, fill=C.WHITE, line=C.LINE)
    semantic_tag(slide, 7.25, 1.72, "THIS REPORT", "evidence", w=1.25)
    textbox(slide, 7.25, 2.18, 4.9, 0.42, "Validation goal", size=18, color=C.BLUE, bold=True)
    multi_text(slide, 7.25, 2.75, 4.8, 1.55, ["Build metrics that explain atomic behavior.", "Use controlled experiments to test the bottleneck hypothesis.", "Only adopt changes that improve clean timestamp runs."], size=13.5, bullet=True)
    textbox(slide, 0.85, 6.1, 11.3, 0.55, "This report turns the atomic-traffic hypothesis into an evidence-backed bottleneck diagnosis.", size=14.5, color=C.INK, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Measurement framework: separate timing from structure", page)
    rows2 = [
        ["Clean timestamp runs", "Performance result", "Median compute_ms after warmup", "Duration and speedup"],
        ["Counter-enabled runs", "Bottleneck diagnosis", "CAS fail ratio, backlog, ready-spin, empty-pop", "Queue contention behavior"],
        ["RGP occupancy", "Profiler context", "Wavefront view and event timing", "Tool interpretation boundary"],
    ]
    add_table(slide, 0.75, 1.45, 11.85, 2.25, ["Run type", "Purpose", "Primary readout", "What it supports"], rows2, [2.2, 2.1, 3.6, 3.95], 10.5)
    metric_box(slide, 0.85, 4.25, 3.6, 1.15, "Timing", "Clean timestamp runs quantify performance.", "evidence")
    metric_box(slide, 4.85, 4.25, 3.6, 1.15, "Counters", "Atomic counters explain queue pressure.", "problem")
    metric_box(slide, 8.85, 4.25, 3.6, 1.15, "Profiler", "RGP provides context, not standalone proof.", "neutral")
    textbox(slide, 0.9, 5.95, 11.3, 0.55, "This separation keeps duration numbers comparable while still exposing why each optimization changed the queue behavior.", size=13.5, color=C.INK, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Atomic metrics interpretation", page)
    metric_rows = [
        ["CAS fail per success", "Many lanes or workgroups retry the same queue state.", "High contention; sharding should reduce this."],
        ["Attempts vs success", "Control path does repeated work without throughput.", "Detects wasted polling."],
        ["Queue high-water", "Producer/consumer balance and backlog.", "Shows B/C balance and Q2 pressure."],
        ["Ready-spin / empty-pop", "Consumers claimed or probed before useful work was ready.", "Explains request batching regressions."],
        ["Clean vs counter runs", "Timing and diagnosis are collected separately.", "Keeps performance and cause analysis comparable."],
    ]
    add_table(slide, 0.65, 1.35, 12.05, 3.6, ["Metric", "Interpretation", "Decision use"], metric_rows, [2.45, 4.4, 5.2], 9.8)
    textbox(slide, 0.8, 5.35, 11.6, 0.65, "The bottleneck diagnosis is strongest when clean duration and atomic metrics move in the same direction.", size=14.5, color=C.INK, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Bottleneck timeline: median clean compute_ms", page)
    timeline = [row for row in rows if row["category"] == "main_timeline"]
    add_table(
        slide,
        0.55,
        ...
        7.1,
        3.95,
        ["Stage", "Config", "Median ms"],
        [[row["stage"], row["config"], row["median_compute_ms"]] for row in timeline],
        [1.45, 4.35, 1.1],
        8.9,
    )
    slide.shapes.add_picture(str(assets["timeline"]), Inches(7.9), Inches(1.35), width=Inches(4.8))
    textbox(slide, 0.75, 5.65, 11.3, 0.55, "Every major improvement corresponds to reducing or distributing queue atomic contention.", size=14.5, color=C.GREEN, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Optimization 1: B/C ratio", page)
    bc_rows = []
    ...
    semantic_tag(slide, 5.8, 1.7, "EVIDENCE", "evidence", w=1.0)
    textbox(slide, 5.8, 2.15, 5.95, 0.36, "What this stage proved", size=17, color=C.BLUE, bold=True)
    multi_text(slide, 5.8, 2.65, 5.95, 0.9, ["Increasing Node C writers reduced Q2 backlog.", "C=40/B=56 improved median compute, but queue contention remained the larger bottleneck."], size=12.5, bullet=True)
    ...
    textbox(slide, 6.85, 4.72, 5.1, 0.8, "Interpretation: writer balance matters, but it does not remove the single hot queue state.", size=13.5, color=C.INK, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Optimization 2: Q1 sharding", page)
    ...
    multi_text(slide, 6.55, 3.0, 5.5, 1.35, ["Q1 sharding turns one global atomic hotspot into multiple smaller hotspots.", "After Q1 pressure drops, Q2 queue pressure becomes the next clear bottleneck."], size=12.3, bullet=True)
    metric_box(slide, 0.9, 4.75, 2.65, 1.05, "13.74 ms", "Q1s16/Q2s1 median", "adopted")
    textbox(slide, 4.0, 4.92, 7.8, 0.65, "The improvement stacks with B/C tuning, which supports the contention hypothesis.", size=14, color=C.INK, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Optimization 3: Q2 sharding", page)
    ...
    semantic_tag(slide, 6.25, 1.63, "READOUT", "adopted", w=1.0)
    textbox(slide, 6.25, 2.08, 5.5, 0.38, "Second major structural optimization", size=17, color=C.GREEN, bold=True)
    multi_text(slide, 6.25, 2.6, 5.5, 1.0, ["Q1 sharding exposed Q2 count/head/tail as the next global pressure point.", "Q2 sharding lowered median compute to about 5.57 ms after B/C rebalance."], size=12.3, bullet=True)
    ...
    textbox(slide, 6.95, 4.9, 5.1, 0.65, "The repeated pattern is not general compute optimization. It is queue hotspot removal.", size=14, color=C.INK, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Optimization 4: Q1 lane-pop", page)
    lane_rows = [
    ...
    textbox(slide, 7.45, 1.65, 4.55, 0.36, "Scan-start change", size=17, color=C.BLUE, bold=True)
    ...
    textbox(slide, 0.85, 4.35, 11.4, 0.9, "This is not request batching. It keeps the queue protocol but diversifies where lanes begin scanning, reducing avoidable local shard contention.", size=14.5, color=C.INK, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Optimization 5: high shard scaling", page)
    ...
    textbox(slide, 4.0, 5.45, 7.9, 0.75, "256 shards is the effective range in this architecture. Higher counts begin losing benefit to probing and indexing overhead.", size=14, color=C.INK, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Request batching: why the idea was plausible", page)
    ...
    textbox(slide, 1.05, 3.85, 11.0, 0.38, "Validation question", size=17, color=C.BLUE, bold=True)
    textbox(slide, 1.05, 4.32, 10.9, 0.5, "Does request batching improve clean median and P90 in the same controlled suite, or does it only move the cost into ready-spin and wasted polling?", size=13.5, color=C.INK, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Request batching controlled results", page)
    ...
    add_table(slide, 0.6, 1.25, 6.0, 3.05, ["Default 256 suite", "Samples", "Median", "P90"], default_rows, [1.7, 1.05, 1.5, 1.5], 9.0)
    stress_rows = []
    ...
    add_table(slide, 6.95, 1.25, 5.6, 2.55, ["Stress 16 suite", "Median", "P90"], stress_rows, [2.0, 1.7, 1.7], 9.0)
    metric_box(slide, 0.9, 4.75, 2.7, 1.05, f"{default_off['median_compute_ms']} ms", "Suite-local off baseline", "neutral")
    metric_box(slide, 3.95, 4.75, 2.7, 1.05, "11 ms range", "Default 256 batching regression", "rejected")
    textbox(slide, 7.0, 4.85, 5.2, 0.9, "Conclusion: batching can help selected stress medians, but it does not generalize and can severely regress the default 256-shard setup.", size=13.5, color=C.RED, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Adopted default: current main path", page)
    ...
    textbox(slide, 7.0, 3.45, 5.1, 0.9, "The main path is not a magic parameter. It is the endpoint of a sequence that repeatedly removed queue/global atomic contention.", size=14.5, color=C.INK, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Final confirmation: main integrated repeats", page)
    ...
    textbox(slide, 7.55, 4.75, 4.6, 0.8, "Repeat runs show a stable median near 0.99 ms with P90 close to 1.00 ms.", size=14, color=C.INK, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Key conclusion", page)
    metric_box(slide, 0.95, 1.45, 3.05, 1.25, "Verified", "Atomic traffic was validated as the bottleneck class.", "evidence")
    metric_box(slide, 4.35, 1.45, 3.05, 1.25, "Adopted", "Sharding plus lane-pop is the main optimization path.", "adopted")
    metric_box(slide, 7.75, 1.45, 3.05, 1.25, "Rejected", "Request batching is not adopted for this protocol/workload.", "rejected")
    rect(slide, 0.95, 3.65, 11.1, 1.4, fill=C.WHITE, line=C.LINE)
    textbox(slide, 1.25, 3.95, 10.5, 0.55, "The current architecture can reach about 0.99 ms median, but the remaining design question is whether work can stay more local before spilling to global queues.", size=15.5, color=C.INK, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Next plan: wave-local work retention", page)
    ...
    box_label(slide, 5.0, 1.6, 3.0, 1.2, "Prototype", "Wave-local retention\nspill-to-global fallback", "evidence")
    arrow(slide, 8.2, 2.2, 9.0, 2.2, C.GRAY)
    box_label(slide, 9.15, 1.6, 3.0, 1.2, "Compare", "global atomic count\nspill ratio\ntail stability", "neutral")
    multi_text(slide, 0.95, 3.75, 5.1, 1.2, ["Keep 256-shard main path as the baseline.", "Measure global atomic count, wave-local handoff count, spill-to-global ratio, and compute duration."], size=13, bullet=True)
    multi_text(slide, 6.75, 3.75, 5.1, 1.2, ["Revisit request batching only if the queue protocol changes.", "Keep timing and atomic-counter diagnosis separate in the next experiment."], size=13, bullet=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Appendix: code snippets", page)
...
if (atomicCompSwap(control.q2[shard].count, cur, cur - claim) == cur) {
    ...
    snippets = [
        ("SHARD PUSH", "Shard push / ready publish", snippet1, "adopted"),
        ("Q1 LANE-POP", "Q1 lane-pop start shard", snippet2, "adopted"),
        ("Q2 BATCH CAS", "Request batching CAS claim", snippet3, "rejected"),
    ]
    ...
    slide = blank_slide(prs)
    header(slide, "Appendix: numeric source files", page)
    add_table(
        slide,
        0.65,
        1.35,
    ...
    textbox(slide, 0.85, 5.05, 11.3, 0.65, "Primary performance numbers are sourced from the normalized measurement table.", size=14.5, color=C.BLUE, bold=True)
