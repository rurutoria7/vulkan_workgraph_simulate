...
VERIFY_DIR = MONTHLY / "verification" / "260525_optimization_evaluation_v3"
ASSET_DIR = VERIFY_DIR / "assets"

CSV_OUT = METRICS / "monthly_v3_normalized_measurements_20260525.csv"
NOTES_OUT = METRICS / "monthly_v3_measurement_notes_20260525.md"
PPTX_OUT = DECK_DIR / "260525_MTK_WorkGraph_VulkanPersistentThread_POC_OptimizationEvaluation_v3.pptx"
...
def write_measurement_files(rows: list[dict[str, str]]) -> None:
    ...
    notes = f"""# Monthly v3 Measurement Notes

Date: 2026-05-25

This file defines the measurement rules used by the v3 monthly deck.
...
def header(slide, title: str, page: int):
    ...
    textbox(slide, 9.2, 7.12, 3.3, 0.18, "Optimization Evaluation v3", size=7, color=C.MUTED, align="right")
...
def build_deck(rows: list[dict[str, str]], assets: dict[str, Path]) -> None:
    ...
    multi_text(slide, 7.0, 4.28, 4.9, 0.8, ["Candidate changes must reduce clean duration, queue contention, or both.", "Changes that move cost into polling or ready-spin are rejected."], size=12, bullet=True)
    page += 1
    ...
    multi_text(slide, 7.25, 2.75, 4.8, 1.55, ["Clean timestamp runs set the performance number.", "Counter-enabled runs diagnose CAS retries, backlog, and polling.", "Adopt changes that improve clean timing."], size=13.5, bullet=True)
    ...
    textbox(slide, 0.9, 5.95, 11.3, 0.55, "Separating these runs keeps counter instrumentation out of the performance result.", size=13.5, color=C.INK, bold=True)
    page += 1

    slide = blank_slide(prs)
    header(slide, "Each atomic metric maps to a queue failure mode", page)
    metric_rows = [
        ["CAS fail per success", "Lanes or workgroups retry the same queue state.", "High contention; sharding should reduce this."],
    ...
    textbox(slide, 0.75, 5.65, 11.3, 0.55, "The curve drops at queue split or scan-diversification steps; the adopted path excludes request batching.", size=14.5, color=C.GREEN, bold=True)
    page += 1
    ...
    textbox(slide, 6.85, 4.72, 5.1, 0.8, "Set the worker mix with this result before testing structural queue changes.", size=13.5, color=C.INK, bold=True)
    page += 1
    ...
    multi_text(slide, 6.25, 2.6, 5.5, 1.0, ["The Q2 sweep improves through 16 shards in this stage.", "After B/C rebalance, Q1s16/Q2s16 reaches about 5.57 ms median."], size=12.3, bullet=True)
    ...
    textbox(slide, 7.0, 4.85, 5.2, 0.9, "Stress cases can improve, but the default path moves from 1.94 ms off-baseline into the 11 ms band. That fails the adoption test.", size=13.5, color=C.RED, bold=True)
    page += 1
    ...
    textbox(slide, 7.0, 3.45, 5.1, 0.9, "These defaults preserve the winning mechanisms: split both queues, diversify Q1 scan starts, and keep the B/C mix from the best clean run.", size=14.5, color=C.INK, bold=True)
    page += 1
